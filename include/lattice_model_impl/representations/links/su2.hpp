...
namespace lm_impl {
    namespace link {
        ...
        SU2<T> operator*(const SU2<T> &x, const SU2<T> &y) {
            return SU2<T>(x(0) * y(0) - x(1) * y(1) - x(2) * y(2) - x(3) * y(3),
                          y(0) * x(1) + x(0) * y(1) - x(2) * y(3) + x(3) * y(2),
                          y(0) * x(2) + x(0) * y(2) - x(3) * y(1) + x(1) * y(3),
                          y(0) * x(3) + x(0) * y(3) - x(1) * y(2) + x(2) * y(1));
        }
...
namespace std {
    template<typename T>
    std::string to_string(lm_impl::link::SU2<T> x)
    {
        std::string conf = "";
        for(auto j = 0; j < 4; j++)
    ...
    template<typename T>
    double fabs(lm_impl::link::SU2<T> x)
    {
